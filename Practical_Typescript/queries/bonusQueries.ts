...
export const sumRequestsPerTimeframe = async (
  ...
  let sumRequests: number;
  if (queryResult instanceof Array) {
    sumRequests = JSON.parse(JSON.stringify(queryResult[0])) as number;
    return sumRequests;
  } else {
...
export const avgRequestsPerTimeframe = async (
  dateFrom: Date | string,
  dateTo: Date | string
): Promise<number> => {
  const dateFromMs = new Date(dateFrom).getTime();
  ...
  const queryResult = await query(sql);
  let avgRequests: number;
  if (queryResult instanceof Array) {
    avgRequests = JSON.parse(JSON.stringify(queryResult[0])) as number;
    return avgRequests;
  } else {
    throw new Error("Error");
  }
...
interface MostUsedApiKeyQuery {
  maxWeight: number;
  apiKey: string;
}

export const mostUsedApiKey = async (): Promise<MostUsedApiKeyQuery> => {
  ...
  const queryResult = await query(sql);
  let finalResult: MostUsedApiKeyQuery;
  if (queryResult instanceof Array) {
    finalResult = JSON.parse(
      JSON.stringify(queryResult[0])
    ) as MostUsedApiKeyQuery;
    return finalResult;
  } else {
    throw new Error("Error");
  }
